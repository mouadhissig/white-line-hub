-- Update register_submission to use per-atelier capacity, falling back to global atelier_cap
...
BEGIN
  ...
  IF p_atelier IS NOT NULL AND p_atelier <> '' THEN
    -- Per-atelier cap takes precedence
    SELECT value INTO v_cap FROM public.survey_settings WHERE key = 'atelier_cap_' || p_atelier;
