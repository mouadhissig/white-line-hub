...
serve(async (req) => {
  ...
  try {
    const { adminKey, atelierCap } = await req.json();
    ...
    if (!Number.isInteger(cap) || cap < 1 || cap > 1000) {
      return new Response(
        JSON.stringify({ error: "Capacité invalide (1-1000)." }),
        { status: 400, headers }
      );
    }
    ...
    const { error } = await supabase
      ...
      .upsert({ key: "atelier_cap", value: cap, updated_at: new Date().toISOString() });
    ...
    return new Response(
      JSON.stringify({ success: true, atelierCap: cap }),
      { status: 200, headers }
    );
  } catch (err) {
